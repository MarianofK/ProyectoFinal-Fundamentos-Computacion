...
void DrawPlayer(Player* player) {
    ...
    DrawText(TextFormat("Player 2: %.1f", player2.life), 900, 60, 20, BLACK);
}
...
void UpdatePlayer(Player* player, int playerNumber) {
    ...
    Collision colX = CheckCollisionWithMap(moveX, &player->position, PLAYER_SIZE / 2, false, 0);
    if (!colX.hasColided) {
    ...
    Collision colY = CheckCollisionWithMap(moveY, &player->position, PLAYER_SIZE / 2, false, 0);
    if (!colY.hasColided) {

...
void DrawEndMenu(int x, int y){
    DrawText(TextFormat("El jugador %d ganó", player1.life <= 0 ? 2 : player2.life <= 0 ? 1 : 0), x - 90, y - 50, 20, BLACK);

    btnContinue.x = x - rectangleWidth/2;
    btnContinue.y = y;
    DrawRectangleRec(btnContinue, GetButtonColor(btnContinue));
    DrawText("Continuar", btnContinue.x + (rectangleWidth/2 - 50), btnContinue.y + 15, 20, BLACK);

    btnMainMenu.x = x - rectangleWidth/2;
    btnMainMenu.y = y + 60;
    DrawRectangleRec(btnMainMenu, GetButtonColor(btnMainMenu));
    DrawText("Menu", btnMainMenu.x + (rectangleWidth/2 - 30), btnMainMenu.y + 15, 20, BLACK);
}

...
void DrawSelectionArch(int x, int y, int playerNumber) {
  ...
  DrawText(arch1->name, card1.x + 45, card1.y + ARCH_CARD_HEIGHT - 30, 20,
           BLACK);
  ...
  DrawText(arch2->name, card2.x + 25, card2.y + ARCH_CARD_HEIGHT - 30, 20,
           BLACK);
  ...
  DrawText(arch3->name, card3.x + 35, card3.y + ARCH_CARD_HEIGHT - 30, 20,
           BLACK);
}
...
void DrawArchsToUpgrade(int x, int y, int playerNumber) {
  ...
  case 1:
    if (player1.currentArch->previous != NULL &&
        player1.currentArch->next != NULL) {
      if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) &&
          CheckCollisionPointRec(mouse, card1)) {
        player1.currentArch = player1.currentArch->previous;
        selection = SELECTION_PLAYER_2;
      }
      DrawTexture(player1.currentArch->previous->texture, card1.x, card1.y,
                  WHITE);
  ...
  case 2:
    if (player2.currentArch->previous != NULL &&
      ...
      DrawTexture(player2.currentArch->previous->texture, card1.x, card1.y,
                  WHITE);
